package ovh.fedox.skcord.elements.sections;
...
public class SecSendWebhook extends Section {
    ...
    static {
        ...
        addEntryData(new ExpressionEntryData<>("embeds", null, true, Object.class));
    ...
    private Expression<Object> embeds;
    ...
    public boolean init(Expression<?> @NotNull [] exprs, int matchedPattern, @NotNull Kleenean isDelayed, SkriptParser.@NotNull ParseResult parseResult, @NotNull SectionNode sectionNode, @NotNull List<TriggerItem> triggerItems) {
        ...
        this.embeds = (Expression<Object>) container.getOptional("embeds", false);
        ...
        if (this.message == null && this.embed == null && this.embeds == null) {
            Skript.error("You need to provide a message or an embed to send.");
            return false;
        }

        if (this.embed != null && this.embeds != null) {
            Skript.error("You can't use both 'embed' and 'embeds' at the same time. Use 'embed' for a single embed or 'embeds' for multiple embeds.");
            return false;
        }
    ...
    private void trigger(Event e) {
        ...
        Object[] embedsArray = this.embeds != null ? this.embeds.getArray(e) : null;
        ...
        if (embedsArray != null && embedsArray.length > 0) {

            for (Object obj : embedsArray) {
                if (!(obj instanceof Embed)) {
                    Skript.error("All items in 'embeds' must be of type Embed.");
                    return;
                }
            }

            discordWebHook.setEmbeds(
                    Arrays.stream(embedsArray)
                            .map(obj -> (Embed) obj)
                            .toList()
            );
        }
        ...
        try {
            ...
            discordWebHook.sendToDiscordAsync(webhook, threadIdLong,
                    ...
                    t -> Skript.error("Failed to send webhook message: " + t.getMessage())
            );
